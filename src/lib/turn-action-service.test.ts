...
describe('turn action service', () => {
  ...
  it('spawns a turn event atomically when the GM resolves with an event payload', () => {
    const { sqlite, db } = createTestDatabase();
    seedRealm(db);
    const service = createTurnActionService(db);
    ...
    const updated = service.updateAction('game-1', action.id, 'realm-1', {
      ...
      event: {
        title: 'Trade Pact Signed',
        description: 'Aster and Bastion agree to favourable terms.',
        treasuryDelta: 500,
        turmoilAmount: 2,
      },
    });
    ...
    expect(updated.status).toBe('executed');
    expect(updated.spawnedEventId).toBeTruthy();

    const event = db.select().from(schema.turnEvents)
      .where(eq(schema.turnEvents.id, updated.spawnedEventId!))
      .get();
    ...
    expect(payload.sourceActionId).toBe(action.id);

    const modifiers = JSON.parse(event!.mechanicalEffect!);
    expect(modifiers).toHaveLength(1);
    expect(modifiers[0].treasuryDelta).toBe(500);
    expect(modifiers[0].turmoilSources).toHaveLength(1);
    expect(modifiers[0].turmoilSources[0].amount).toBe(2);

    sqlite.close();
  });

  it('skips event creation when the action already has a spawned event', () => {
    const { sqlite, db } = createTestDatabase();
    seedRealm(db);
    const service = createTurnActionService(db);
    ...
    const action = service.createAction('game-1', 'realm-1', {
      kind: 'political',
      actionWords: ['Investigate'],
      description: 'Investigate corruption in the capital.',
    });
    ...
    const firstUpdate = service.updateAction('game-1', action.id, 'realm-1', {
      role: 'gm',
      label: 'GM',
    }, {
      status: 'executed',
      outcome: 'partial',
      resolutionSummary: 'Evidence was found but inconclusive.',
      event: {
        title: 'Corruption Exposed',
        description: 'Rumours of corruption spread.',
      },
    });

    expect(firstUpdate.spawnedEventId).toBeTruthy();

    const secondUpdate = service.updateAction('game-1', action.id, 'realm-1', {
      role: 'gm',
      label: 'GM',
    }, {
      resolutionSummary: 'Updated notes after further review.',
      event: {
        title: 'Should Not Be Created',
        description: 'This event should be ignored.',
      },
    });

    expect(secondUpdate.spawnedEventId).toBe(firstUpdate.spawnedEventId);

    const allEvents = db.select().from(schema.turnEvents)
      .where(eq(schema.turnEvents.gameId, 'game-1'))
      .all();
    expect(allEvents).toHaveLength(1);
    expect(allEvents[0].title).toBe('Corruption Exposed');

    sqlite.close();
  });

  it('creates event without mechanical effects when deltas are zero', () => {
    const { sqlite, db } = createTestDatabase();
    seedRealm(db);
    const service = createTurnActionService(db);
    ...
    const action = service.createAction('game-1', 'realm-1', {
      kind: 'political',
      actionWords: ['Declare'],
      description: 'Declare a day of celebration.',
    });
    ...
    const updated = service.updateAction('game-1', action.id, 'realm-1', {
      ...
      status: 'executed',
      outcome: 'success',
      resolutionSummary: 'The realm celebrates.',
      event: {
        title: 'Day of Celebration',
        description: 'A joyous day across the realm.',
        treasuryDelta: 0,
        turmoilAmount: 0,
      },
    });
