...
const BORDER_SNAP_DISTANCE = 90;
const TERRAIN_CODE_TO_TYPE = {
  1: 'flat_farmland',
  2: 'hills',
  3: 'mountains',
  4: 'hills_forest_deciduous',
  5: 'mountains_forest_deciduous',
  6: 'flat_desert_rocky',
  7: 'flat_forest_deciduous',
  8: 'flat_grassland',
  9: 'flat_desert_sandy',
  10: 'flat_forest_deciduous_heavy',
  11: 'flat_swamp',
  12: 'flat_forest_jungle',
  13: 'badlands',
  14: 'hills_grassy',
  15: 'hills_forest_jungle',
  16: 'mountains_forest_jungle',
};
...
function getPointToSegmentDistance(point, start, end) {
  const segmentLengthSquared = getDistanceSquared(start, end);
  ...
  const closestPoint = {
    x: start.x + ((end.x - start.x) * clampedProjection),
    y: start.y + ((end.y - start.y) * clampedProjection),
  };

  return Math.sqrt(getDistanceSquared(point, closestPoint));
}
...
function formatTerrainTypeName(terrainType) {
  return terrainType.replace(/_/g, ' ');
}
...
function buildBlockedEdges(parsedMap, landTilesById) {
  ...
  for (const tile of landTilesById.values()) {
    ...
    for (const neighbor of getOffsetNeighbors(tile.column, tile.row)) {
      ...
      edgeCandidates.push({
        id: getEdgeId(tileId, neighborId),
        centerLine: [tileCenter, neighborCenter],
        midpoint: {
          x: (tileCenter.x + neighborCenter.x) / 2,
          y: (tileCenter.y + neighborCenter.y) / 2,
        },
      });
    }
  }
  ...
  for (const edge of edgeCandidates) {
    const isBlocked = shapeSegments.some(([start, end]) => (
      segmentsIntersect(edge.centerLine[0], edge.centerLine[1], start, end)
      || getPointToSegmentDistance(edge.midpoint, start, end) <= BORDER_SNAP_DISTANCE
    ));
...
function buildTerritoryDescriptions(territoriesByKey, landHexes) {
  ...
  return territoriesByKey.map((territory) => {
    ...
    const dominantTerrain = [...counts.entries()]
      ...
      .slice(0, 3)
      .map(([terrain]) => formatTerrainTypeName(terrain));
...
export function serializeCuratedMapDefinition(definition, metadata = {}) {
  ...
  return [
    ...
    `export const WORLD_V1_MAP_DEFINITION = ${serializedDefinition} satisfies CuratedMapDefinition;`,
    '',
  ].join('\n');
}
